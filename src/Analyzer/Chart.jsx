...
const Chart = ({ prices }) => {
  ...
  const getDates = (prices) => {
    if (prices)
      return prices.map((priceArray) => unixToLocaleString(priceArray[0]));
  ...
  const getTimeRangeAsLabel = (prices) => {
    ...
    return "Bitcoin Analyzer"; //time range is not set
  };
  ...
  return (
    <AnalyzerContainer className="chart">
      <HeadingSm className="chart-title">
        {getTimeRangeAsLabel(prices)}
      </HeadingSm>
